...
int main(int argc, char *const *argv)
{
    ...
    if (6 != argc)
    {
        ...
        fprintf(stderr, "        S1 and S2 saturated pixels percentage [0...100[\n");
        return EXIT_FAILURE;
    }
    ...
    for (n=0; n < (int) nx*ny; n++) {
      fI=((float) data2[n]+(float) data2[n+nx*ny]+(float) data2[n+2*nx*ny])/3.;
      ii=(int) (fI+0.5f);
    ...
    /*compute the numbers of pixels whose value is zero in each channel*/
       ...
       for(n=0; n < (int) nx*ny; n++) {
         if(data2[n] == 0)  per_r0++; 
         if(data2[n+nx*ny] ==0) per_g0++; 
         if( data2[n+2*nx*ny]==0) per_b0++; 
        }
     ...
     /* compute the minimum value of the gray level such that at most s1% of pixels sature to zero in one channel*/
      ...
      if( per_r0 > nx*ny*(s1/100.) || per_g0 > nx*ny*(s1/100.) || per_b0 > nx*ny*(s1/100.) ) ming=0;
      else {
	    ...
	    (void) normalize_histo_u8_gray(Inew, nx*ny, 0,255, ming, maxg);
            ...
            for(n=0; n < (int) nx*ny; n++) {
             if(data3[n] == 0) per_r0++;
             if( data3[n+nx*ny]== 0) per_g0++;
             if( data3[n+2*nx*ny] == 0) per_b0++;
           }
          
           if( per_r0 > nx*ny*(s1/100.) || per_g0 > nx*ny*(s1/100.) || per_b0 > nx*ny*(s1/100.) ) ming--;

            memcpy(Inew, I, nx*ny*sizeof(unsigned char));
           
        } while(  (per_r0 >  nx*ny*(s1/100.) || per_g0 > nx*ny*(s1/100.) || per_b0 > nx*ny*(s1/100.)) );
       ...
       per_r0=0; per_g0=0; per_b0=0;
       for(n=0; n < (int) nx*ny; n++) {
         if(data2[n] == 255)  per_r0++; 
         if(data2[n+nx*ny] == 255) per_g0++; 
         if( data2[n+2*nx*ny]== 255) per_b0++; 
        }
      

        /* compute the maximum value of the gray level such that at most s2% of pixels sature to 255 in one channel*/

        if( per_r0 > nx*ny*(s2/100.) || per_g0 > nx*ny*(s2/100.) || per_b0 > nx*ny*(s2/100.) ) maxg=255;
        else {
        minmax_histo_u8(I,nx*ny, nx*ny*(s1/100.), nx*ny*(s2/100.), &min, &maxg);
        ...
        /* compute each channel for the new gray*/
         ...
         /*compute the numbers of pixels whose value is 255 in each channel */
            per_r0=0; per_g0=0; per_b0=0;
            for(n=0; n < (int) nx*ny; n++) {
            if(data3[n] == 255) per_r0++;
            if( data3[n+nx*ny] == 255) per_g0++;
            if( data3[n+2*nx*ny] == 255) per_b0++;
           }
          
           if( per_r0 >  nx*ny*(s2/100.) || per_g0 > nx*ny*(s2/100.) ||  per_b0 > nx*ny*(s2/100.) ) maxg++;
           memcpy(Inew, I, nx*ny*sizeof(unsigned char));
        } while( (per_r0 > nx*ny*(s2/100.) ||  per_g0 > nx*ny*(s2/100.) ||  per_b0 > nx*ny*(s2/100.)) );
